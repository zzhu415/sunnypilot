        ...
        for b in buttonEvents:
          if not b.pressed:
            if b.type == car.CarState.ButtonEvent.Type.accelCruise:
              ...
              if (not fastMode):
                v_cruise_kph -= 1
    v_cruise_kph = clip(v_cruise_kph, 40 if not is_metric else 30, V_CRUISE_MAX)
...
def initialize_v_cruise(v_ego, buttonEvents, v_cruise_last):
  ...
  return int(round(clip(v_ego * CV.MS_TO_KPH, 40 if not is_metric else 30, V_CRUISE_MAX)))
